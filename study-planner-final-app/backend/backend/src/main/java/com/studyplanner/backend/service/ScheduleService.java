...
public class ScheduleService {
    ...
    public List<ScheduleDto.ScheduleResponse> getUserSchedules(String username) {
        ...
        return scheduleRepository.findByUserIdOrderByNameAsc(user.getId())
    ...
    public ScheduleDto.ScheduleResponse setActiveSchedule(Long id, String username) {
        ...
        if(!schedule.getUser().getUsername().equals(username)) throw new RuntimeException("Not authorized");
    ...
    public ScheduleDto.SlotResponse addSlot(Long scheduleId, String username, ScheduleDto.SlotRequest slotRequest) {
        ...
        // ΕΝΗΜΕΡΩΜΕΝΑ ΠΕΔΙΑ
        scheduleSlot.setTitle(slotRequest.getTitle());
        scheduleSlot.setIsRecurring(slotRequest.getIsRecurring());
        ...
        scheduleSlot.setWeekStart(slotRequest.getWeekStart());
    ...
    public void removeSlot(Long scheduleId, Long slotId, String username) {
        Schedule schedule = scheduleRepository.findById(scheduleId).orElseThrow(()->new RuntimeException("Schedule not found"));
        if(!schedule.getUser().getUsername().equals(username)) throw new RuntimeException("Not authorized");
        ScheduleSlot slot = scheduleSlotRepository.findById(slotId).orElseThrow(()->new RuntimeException("Slot not found"));
        if(!slot.getSchedule().getId().equals(schedule.getId())) throw new RuntimeException("Slot belongs to another schedule");
    ...
    private ScheduleDto.SlotResponse toSlotResponse(ScheduleSlot slot) {
        ...
        // ΕΝΗΜΕΡΩΜΕΝΑ ΠΕΔΙΑ
        response.setTitle(slot.getTitle());
        ...
        response.setWeekStart(slot.getWeekStart());
